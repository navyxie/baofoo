...
BaofooSdk.prototype.doPay = function(payJsonData, cb) {
    ...
    this.log('===============baofoo->doPay->dataContentJson->', dataContentJson);
    this._post(dataContentJson, cb);
}
        ...
        function(bind_id, cb) {
            _this.sendMessage({
                ...
                if (!err && UTIL.isSuccess(data)) {
                    _this.log('===============baofoo->sendPayMessage->acc_no:', msgJsonData.acc_no, '->bind_id:', bind_id, '短信认证码下发成功');
                    return cb(null, data);
                }
                _this.log('===============baofoo->sendPayMessage->acc_no:', msgJsonData.acc_no, '->bind_id:', bind_id, '短信认证码下发失败，请重试', data);
...
BaofooSdk.prototype.getBind_Id = function(bindCardData, cb) {
    ...
    async.waterfall([
            ...
            _this.queryBindCard({
                ...
                if (!err) {
                    if (UTIL.isSuccess(data)) {
                        ...
                        _this.log('===============baofoo->getBind_Id(查询绑卡):success->acc_no:',bindCardData.acc_no,'->bind_id:', bind_id);
                        cb(null, true);
                    } else {
                        _this.log('===============baofoo->getBind_Id(查询绑卡):fail->acc_no:',bindCardData.acc_no);
                        cb(null, false);
                    }
                } else {
                    _this.log('===============baofoo->getBind_Id(查询绑卡):error->acc_no:',bindCardData.acc_no);
                    cb(err)
                }
            })
        },
        ...
        function(isBindCard, cb) {
            if (isBindCard) {
                _this.log('===============baofoo->acc_no:',bindCardData.acc_no,'绑卡成功');
                return cb(null);
            }
            _this.bindCard(cloneData, function(err, data) {
                _this.log('===============baofoo->acc_no:',bindCardData.acc_no,'重新绑卡->', data);
                if (!err) {
                    if (UTIL.isSuccess(data)) {
                        bind_id = data.bind_id;
                        cb(null);
...
BaofooSdk.prototype.decryptByPublicKey = function (decrypted_str) {
    return this.rsa.decryptByPublicKey(decrypted_str);
}
BaofooSdk.prototype.log = function () {
    if (this.config.debug) {
        console.log(chalk.yellow(nodeUtil.format.apply(null, arguments)));
        console.log('\n');
    }
}
