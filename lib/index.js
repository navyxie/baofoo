...
BaofooSdk.prototype.sendPayMessage = function(msgJsonData, cb) {
    ...
    async.waterfall([
        ...
        //发送短信认证码
        function(bind_id,cb){
            _this.sendMessage({acc_no:msgJsonData.acc_no,bind_id:bind_id,txn_amt:msgJsonData.txn_amt},function(err,data){
...
BaofooSdk.prototype.getBind_Id = function(bindCardData,cb){
    ...
    async.waterfall([
        //查询绑卡
        function(cb){
            _this.queryBindCard({acc_no:bindCardData.acc_no},function(err,data){
                if(!err){
